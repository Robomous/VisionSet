...
| YOLO smoke (ultralytics) | `uv sync --group yolo && uv run pytest tests/formats/test_yolo_smoke.py` — its own group because ultralytics brings torch; skips locally without it, and CI sets `VISIONSET_REQUIRE_ULTRALYTICS=1` so a broken install goes red |
