...
export function createRuntimeClient(
  ...
  channel.onMessage((message) => {
    ...
    operation.detach();
    if (reply.kind === "error") operation.reject(errorOf(reply));
    else operation.resolve(valueOf(reply));
  });

  channel.onError(() => {
    settleAll(new InferenceRuntimeError("worker-initialization-failed"));
  });
  ...
  const readyOperation = send<readonly ExecutionProvider[]>((id) => {
    post({
      kind: "configure",
      id,
