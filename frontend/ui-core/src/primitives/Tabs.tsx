 ...
 * which one is navigation. So #182 made the default `underline`: a row on a
 ...
 *
 * `segmented` survived that pass for exactly one caller: the annotator's 288px side
 * panel, whose two equal halves — **Objects | Labels** — really were a switch. #368
 * deleted the switch. The panel is one Annotations view now, class selection having
 * moved to the top bar, so the variant has no call site and it is gone rather than
 * kept warm: a `cva` with one member, a context that always resolves to the same
...
//
// No weight bump on top of the base `font-medium` (#323): the rule is near-black
