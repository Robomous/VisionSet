...
| `origin-hub` / `origin-custom` / `origin-robomous` | Where a model's weights come from, as a **surface** — the fill of a card on the Models page — soft yellow for the hub, soft blue for the user's own, soft orange on the brand hue for the Robomous registry. Surfaces only: ink stays `foreground`, and an origin is a kind, never a state, so these never stand in for a status. `origin-robomous` is its own token; `brand` stays identity-only | `oklch(0.97 0.045 95)` / `oklch(0.965 0.03 240)` / `oklch(0.965 0.035 32.3)` | `oklch(0.27 0.04 95)` / `oklch(0.26 0.04 240)` / `oklch(0.27 0.04 32.3)` |
