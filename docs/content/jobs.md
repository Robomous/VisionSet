...
is the consequence of a batch-level pre-label action rather than a per-asset one. `pre_labeled →
annotated` is the one edge with two ways to ride it: an edit, which is derived, and `confirm`,
which a person clicks to keep a model's labels as the frame's own - labels exist either way, the
marker records judgment. Offering any of the other four as a control would mean changing a marker
while the labels stay put.
