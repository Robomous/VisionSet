...
A **source** records raw data offered to a project: either a directory of still images or a
video file. A source is not annotatable and stores no pixels. Ingest *materializes* assets from
the source, which retains their origin.
