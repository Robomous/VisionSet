...
the proxy it looks like, because registration is idempotent on a source's origin key and is
never rewritten - and a `VIDEO` source is not even that, since every import declares a fresh
opaque locator, so no two clips can share a row to date-stamp. Sorting them last is the only reading that degrades quietly: treating
