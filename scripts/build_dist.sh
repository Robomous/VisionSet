...
# nothing else — and it installs, and it starts, and `/ui/` answers 404 naming a
...
# built without `base: "/ui/"` references `/assets/...`, which the SPA fallback
# answers with `index.html` at **200**, so the page loads blank rather than
# failing. Cheap to check, impossible to notice later.
if ! grep -q '"/ui/assets/' "$index" && ! grep -q "'/ui/assets/" "$index"; then
  echo "error: $index does not reference /ui/assets — was it built with the dev base?" >&2
  exit 1
fi
