...
- **Where the harness kills long-running commands, run the gate in stages rather than fighting the ceiling.** The observed limit is ~10 minutes, the kill takes the whole process group, and every way out of it fails: `run_in_background`, a watcher, and `nohup … & disown` all die at the same point (and `setsid` does not exist on macOS, so that spelling dies instantly and silently). The stages that fit: pytest split by test directory, then `ruff` / `mypy` / `lint-imports`, then frontend, then browser. **Record every stage's exit code verbatim in the PR body** — a staged gate whose stages are undocumented is indistinguishable from a partial one, the same false-calm failure as #336. And never pipe a runner through `tail` to dodge the ceiling: the repo forbids it, and it swallows the summary line along with the exit code. — #344
...
- **The browser stages are single-occupancy per machine**, until #346 lands: the e2e suite binds 5273 with `--strictPort` and the cycle suite binds 8123, neither overridable. Two worktrees must not run their browser stages at the same time — which is a standing hazard, because worktree isolation is what this protocol produces by design. **A stage that fails far faster than its normal runtime is a setup collision, not a test failure**: check for a competing worktree (`lsof -nP -iTCP:5273 -sTCP:LISTEN`, and read the cmdline for the path that owns it) before debugging a single test. The subtle mode is the one that costs time — a foreign server answering `/health` on 8123 first makes Playwright declare readiness against a process that never wrote this run's token, so the failure points at token logic. It is the same collision. **Never kill a process belonging to another session**; wait for the ports to release and rerun the stage. — #344
