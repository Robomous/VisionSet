...
4. **Raw refusal codes as UI.** Refusals render through the shared code→prose vocabulary (one map, product-wide — F16). A bare `BATCH_NOT_IN_ANNOTATION` badge is not a message. The prose is always the visible notice; the code itself survives only where a bug report can reach it, and where that is depends on the surface — an inline notice (`EditorNotice`) puts it in the DOM `title` tooltip, quotable, never rendered; a full error state (`ErrorState`) puts it in a secondary mono meta line, de-duplicated against the sentence; a bare `Alert` does not carry it at all. A visible `Badge` beside the raw message, which six sites still do, is the fourth shape and is not yet settled. A code as an `Alert` heading is the shape the vocabulary exists to end.
